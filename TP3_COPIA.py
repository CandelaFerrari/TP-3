...
Original file is located at
    https://colab.research.google.com/drive/10LpKhDr6erxF-tK9O6NhfKeoNVJLgmHW
"""
...
def alta_productos():
    ...
    while rta=='S':
        ...
        while len(pro)<3 or len(pro)>=15:
            pro=input("ERROR. Ingrese el producto [hasta 15 caracteres] :  ")
        pro=pro.upper()
        RegProd=prod()
...
def mostrar_cupos(): # MUESTRO CUPOS PARA EL DIA DE HOY
    global AF_OP, AL_OP
    ff = fecha_hoy()
    os.system('cls')
    t=os.path.getsize(AF_OP)
    print("Cupos para el",ff)
    if t==0:
      print("No hay cupos para mostrar")
      os.system('pause')
    else:
      rOper=oper()
      print("-----------------------------------------")
      print("Cod -  Patente - Estado")
      AL_OP.seek(0)
      while AL_OP.tell()<t:
          rOper=pickle.load(AL_OP)
          if rOper.fecha == ff:
            print(rOper.cod_prod, " - ", rOper.pat, "-  ", rOper.est)
    os.system('pause')

def entrega_cupos():
    global AF_OP, AL_OP
    ke="S"
    cantprod=str(cant_prod())
    print("ENTREGA DE CUPOS")
    while ke == "S":
      os.system('cls')
      rOp=oper()
      rOp.pat=validarPatente() 
      e = Busco_p(rOp.pat) #e = 1 cupo otorgado
      if e ==1:
      ...
      while ke == "S" and e!=1:
        ...
        while rOp.cod_prod<"1" or rOp.cod_prod>cantprod:
          rOp.cod_prod=input("Ingrese el código del producto → ")
        rOp.est="P"
        AL_OP.seek(0,2)
        formatOp(rOp)
        ...
        ke=str.upper(input(print("Desea ingresar otro camion? S-si N-no →\n")))
        e=1
...
AF_OP = "tpn3\\TP3\\OPERACIONES.DAT"
AF_PROD = "tpn3\\TP3\\PRODUCTOS.DAT"
AF_RUBRO = "tpn3\\TP3\\RUBROS.DAT"
AF_RUBROP = "tpn3\\TP3\\RUBXPROD.DAT"
AF_SILOS = "tpn3\\TP3\\SILOS.DAT"

if not os.path.exists('tpn3\\TP3'):
    os.makedirs('tpn3\\TP3')
